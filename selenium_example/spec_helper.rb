...
RSpec.configure do |c| 
    ...
    case config[:host] when 'saucelabs'
        caps = Selenium::WebDriver::Remote::Capabilities.send(config[:browser_name]) caps[:browser_version] = config[:browser_version]
        ...
        when 'localhost'
            ...
            if service
                ...
                @driver = Selenium::WebDriver.for config[:browser_name].to_sym end
            end 
        end

        c.after do |example| begin
